...
export default function generateCodeGlobalStyle(): string {
  ...
  _forEach({
    ...
    _forEach(variables, (_v: string, variableKey: string): void => {
      pushCode(generator, `${nameCssVar(upperWhatKey, variableKey)}: ${buildInterpolation([upperWhatKey, variableKey])};`);
    });
  });
  
  _forEach(SIZE, (_v, key: string) => {
    if (/^BORDER_RADIUS_/.test(key)) { // TODO 所有 size？
      pushCode(generator, `${nameCssVar('SIZE', key)}: ${buildInterpolation(['SIZE', key], 'px')};`);
    }
  });
