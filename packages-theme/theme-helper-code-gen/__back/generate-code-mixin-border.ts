...
export default function generateCodeMixinBorder(): string {
  const generator: ICodeGenerator = {
    generator: 'generate-code-mixin-border',
  ...
  _forEach(COLOR, (_v: string, variableKey: string): void => {
    if (/^BORDER_/.test(variableKey)) {
      ...
      pushCode(generator, `export const ${nameConstStyledMixin(variableKey, 'Color')} = css\`
${cssCode}
\`;`);
      BUTTON_ALTS.forEach(borderAlt => {
        ...
        pushCode(generator, `export const ${nameConstStyledMixin(variableKey, borderAlt)} = css\`
${cssCode2}
\`;`);
      });
    }
