// All functions that need access to the editor's state live inside the CodeMirror function.
// Below that, at the bottom of the file, some utilities are defined.

// CodeMirror is the only global var we claim
var CodeMirror = (function() {
	// This is the function that produces an editor instance. It's closure is used to store the editor state.
	function CodeMirror(place, givenOptions) {
		...
		var wrapper = targetDocument.createElement("div");// The element in which the editor lives.
		...
		wrapper.innerHTML = ["<div style=\"overflow: hidden; position: relative; width: 1px; height: 0px;\">",// Wraps and hides input textarea
					"<textarea style=\"position: absolute; width: 2px;\" wrap=\"off\"></textarea>",
				"</div>",
		...
		// constructor).
		var mode, lines = [ new Line("") ], work, _history = new History(), focused;
		loadMode();
		...
		// Selection-related flags. shiftSelecting obviously tracks whether the user is holding shift.
		...
		// editing will hold an object describing the things we put in the textarea, to help figure out whether something changed.
		...
		// Gecko browsers fire contextmenu *after* opening the menu, at which point we can't mess with it anymore. Context menu is
		...
		// The instance object that we'll return. Mostly calls out to local functions in the CodeMirror function. Some do some extra
		// range checking and/or clipping. operation is used to wrap the call so that changes it makes are tracked, and the display is
		// updated afterwards.
		var instance = {
			getValue: getValue,
			...
			historySize: function() {
				return {
					undo: _history.done.length,
					redo: _history.undone.length
				};
			},
			...
			addWidget: function(pos, node, scroll, where) {
				...
				if (where == "over") {
					...
					var vspace = Math.max(scroller.offsetHeight, lines.length * lineHeight()), hspace = Math.max(code.clientWidth, lineSpace.clientWidth) - paddingLeft();
					if (pos.yBot + node.offsetHeight > vspace && pos.y > node.offsetHeight) {
						t = pos.y - node.offsetHeight;
					}
					if (l + node.offsetWidth > hspace) {
						l = hspace - node.offsetWidth;
					}
				}
				node.style.top = (t + paddingTop()) + "px";
				node.style.left = (l + paddingLeft()) + "px";
				if (scroll) {
					scrollIntoView(l, t, l + node.offsetWidth, t + node.offsetHeight);
				}
			},
		...
		function onMouseDown(e) {
			...
			// And then we have to see if it's a drag event, in which case the dragged-over text must be selected.
			function end() {
				focusInput();
				updateInput = true;
				move();
		...
		function onKeyDown(e) {
			...
			if (mod && ((code == 36 || code == 35) || mac && (code == 38 || code == 40))) {// ctrl-home/end cmd-up/down
			...
			if (!options.readOnly) {
				...
				if (!anyMod && code == 9 && handleTab(e.shiftKey)) {// tab
					return e_preventDefault(e);
				}
				...
				if (mod && ((e.shiftKey && code == 90) || code == 89)) {// ctrl-shift-z, ctrl-y
					redo();
					return e_preventDefault(e);
				}
			}
			...
			// Key id to use in the movementKeys map. We also pass it to fastPoll in order to "self learn".
			// We need this because reducedSelection, the hack where we collapse the selection to its start when it is inverted and a movement
			// key is pressed (and later restore it again), shouldn't be used for non-movement keys.
		...
		function updateLines(from, to, newText, selFrom, selTo) {
			if (_history) {
				...
				_history.addChange(from.line, newText.length, old);
				while (_history.done.length > options.undoDepth) {
					_history.done.shift();
				}
			}
		...
		function undo() {
			unredoHelper(_history.done, _history.undone);
		}
		function redo() {
			unredoHelper(_history.undone, _history.done);
		}
		...
		function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
			...
			// Add these lines to the work array, so that they will be highlighted. Adjust work lines if lines were added/removed.
		...
		// Inspects the textarea, compares its state (content, selection) to the data in the editing variable, and updates the editor
		...
		function readInput() {
			...
			var from = computeOffset(sr.start, editing.from), to = computeOffset(sr.end, editing.from);
			// Here we have to take the reducedSelection hack into account, so that you can, for example, press shift-up at the start of
			...
			// In some cases (cursor on same line as before), we don't have to update the textarea content at all.
			if (from.line == to.line && from.line == sel.from.line && from.line == sel.to.line && !shiftSelecting) {
				updateInput = false;
			}
			
			// Magic mess to extract precise edited range from the changed string.
			if (changed) {
		...
		// Set the textarea content and selection range to match the editor state.
		...
		function updateDisplay(changes) {
			...
			// First create a range of theoretically intact lines, and punch holes in that using the change info.
			...
			// Then, determine which lines we'd want to see, and which updates have to be made to get there.
			var visible = visibleLines();
			var from = Math.min(showingFrom, Math.max(visible.from - 3, 0)), to = Math.min(lines.length, Math.max(showingTo, visible.to + 3)), updates = [], domPos = 0, domEnd = showingTo - showingFrom, pos = from, changedLines = 0;
			...
			// If more than 30% of the screen needs update, just do a full redraw (which is quicker than patching)
			if (changedLines > (visible.to - visible.from) * .3) {
				refreshDisplay(from = Math.max(visible.from - 10, 0), to = Math.min(visible.to + 7, lines.length));
			} else {// Otherwise, only update the stuff that needs updating.
				patchDisplay(updates);
			}
			lineDiv.style.display = "";
			
			// Position the mover div to align with the lines it's supposed to be showing (which will cover the visible display)
			...
			// Since this is all rather error prone, it is honoured with the only assertion in the whole file.
		...
		function patchDisplay(updates) {
			// Slightly different algorithm for IE (badInnerHTML), since there .innerHTML on PRE nodes is dumb, and discards whitespace.
			var sfrom = sel.from.line, sto = sel.to.line, off = 0, scratch = badInnerHTML && targetDocument.createElement("div");
		...
		function setSelection(from, to, oldFrom, oldTo) {
			...
			// Some ugly logic used to only mark the lines that actually did see a change in selection as changed, rather than the whole selected range.
			if (oldFrom == null) {
		...
		function posFromMouse(e, liberal) {
			...
			// This is a mess of a heuristic to try and determine whether a scroll-bar was clicked or not, and to return null if one was (and !liberal).
		...
		function onContextMenu(e) {
			...
			input.style.cssText = "position: fixed; width: 30px; height: 30px; top: " + (e.clientY - 5) + "px; left: " + (e.clientX - 5) + "px; z-index: 1000; background: white; "
		...
		// Finds the line to start with when starting a parse. Tries to find a line with a stateAfter, so that it can start with a valid state.
		...
		// Operations are used to wrap changes in such a way that each change won't have to update the cursor and display (which would
		// be awkward, slow, and error-prone), but instead updates are batched and then all combined and executed at once.
		...
		function endOperation() {
			...
			// updateInput can be set to a boolean value to force/prevent an update.
		...
		function SearchCursor(query, pos, caseFold) {
			...
			// It takes a position and a direction, and returns an object describing the next occurrence of the query, or null if no more matches were found.
			if (typeof query != "string") {// Regexp match
				this.matches = function(reverse, pos) {
					if (reverse) {
						...
						var line = lines[pos.line].text.slice(pos.ch), match = line.match(query), start = match && pos.ch + line.indexOf(match[0]);
					}
					if (match) {
						return {
							from: {
		...
		SearchCursor.prototype = {
			...
			find: function(reverse) {
				var me = this, pos = clipPos(reverse ? this.pos.from : this.pos.to);
	...
	Line.prototype = {
		...
		// Produces an HTML fragment for the line, taking selection, marking, and highlighting into account.
	...
	// The history object "chunks" changes that are made close together and at almost the same time into bigger undoable units.
	function History() {
		this.time = 0;
		this.done = [];
		this.undone = [];
