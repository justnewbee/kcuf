CodeMirror.defineMode("plsql", function(config, parserConfig) {
	var keywords = parserConfig.keywords, functions = parserConfig.functions, types = parserConfig.types, sqlplus = parserConfig.sqlplus, multiLineStrings = parserConfig.multiLineStrings;
	var isOperatorChar = /[+\-*&%=<>!?:\/|]/;
	...
	function tokenBase(stream, state) {
		...
		} else if (ch == "/") {// multi line comment or simple operator?
			if (stream.eat("*")) {
		...
		} else if (ch == "-") {// single line comment or simple operator?
			if (stream.eat("-")) {
				stream.skipToEnd();
