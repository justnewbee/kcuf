...
CodeMirror.defineMode("coffeescript", function(conf) {
	...
	var singleOperators = new RegExp("^[\\+\\-\\*/%&|\\^~<>!\?]"),
		...
		commonKeywords = [ "break", "by", "catch", "class", "continue", "debugger", "delete", "do", "else", "finally", "for", "in", "of", "if", "new", "return", "switch", "then", "this", "throw", "try", "unless", "when", "while", "until", "loop" ],
		keywords = wordRegexp(commonKeywords),
		stringPrefixes = new RegExp("^(([rub]|(ur)|(br))?('{3}|\"{3}|['\"]))", "i"),
		commonConstants = [ "Infinity", "NaN", "undefined", "true", "false" ],
	...
	function tokenLexer(stream, state) {
		...
		if (current === "@") {
			...
			return style === "variable" ? "variable-2" : ERRORCLASS;
		}
		...
		if ((current === "->" && !state.lambda && state.scopes[0].type == "coffee" && stream.peek() === "") || style === "indent") {
			indent(stream, state);
		}
