CodeMirror.defineMode("rst", function(config, options) {
	...
	function getMode(mode) {
		return hasMode(mode) ? CodeMirror.getMode(config, mode) : null;
	}
	
	var verbatimMode = getMode(options.verbatim),
		pythonMode = getMode("python"),
		reSection = /^[!"#$%&'()*+,-.\/:;<=>?@[\\\]^_`{|}~]/,
		reDirective = /^\s*\w([-:.\w]*\w)?::(\s|$)/,
		reHyperlink = /^\s*_[\w-]+:(\s|$)/,
		reFootnote = /^\s*\[(\d+|#)\](\s|$)/,
		reCitation = /^\s*\[[A-Za-z][\w-]*\](\s|$)/,
		reFootnoteRef = /^\[(\d+|#)\]_/,
		reCitationRef = /^\[[A-Za-z][\w-]*\]_/,
		reDirectiveMarker = /^\.\.(\s|$)/,
		reVerbatimMarker = /^::\s*$/,
		rePreInline = /^[-\s"([{<\/:]/,
		rePostInline = /^[-\s`'")\]}>\/:.,;!?\\_]/,
		reEnumeratedList = /^\s*((\d+|[A-Za-z#])[.)]|\((\d+|[A-Z-a-z#])\))\s/,
		reBulletedList = /^\s*[-\+\*]\s/,
		reExamples = /^\s+(>>>|In \[\d+\]:)\s/;
	...
	function normal(stream, state) {
		...
		if (sol && (stream.match(reEnumeratedList) || stream.match(reBulletedList))) {
	...
	function inline(stream, state) {
		var ch = stream.next(), token = state.ctx.token;
