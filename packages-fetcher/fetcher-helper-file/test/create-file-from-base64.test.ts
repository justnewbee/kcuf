...
describe('base64ToFile', () => {
  test('returns a File with mime from the data url', async () => {
    const file = createFileFromBase64(`data:text/plain;base64,${HELLO_BASE64}`, 'greeting.txt');
  ...
  test('appends extension when filename has none and mime is known', () => {
    const file = createFileFromBase64(`data:image/png;base64,${PNG_BASE64}`, 'photo');
  ...
  test('uses the default filename when none provided', () => {
    const file = createFileFromBase64(`data:image/png;base64,${PNG_BASE64}`);
