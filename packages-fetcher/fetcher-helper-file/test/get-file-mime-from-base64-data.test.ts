...
describe('getFileMimeFromBase64Data', () => {
  test('detects PNG (full 8-byte magic)', () => {
    expect(getFileMimeFromBase64Data(PNG_HEADER)).toBe('image/png');
  });
  ...
  test('does not currently detect GIF from raw bytes', () => {
    expect(getFileMimeFromBase64Data(GIF_HEADER)).toBe('');
  });
  
  test('does not currently detect JPEG from raw bytes', () => {
    expect(getFileMimeFromBase64Data(JPEG_HEADER)).toBe('');
  });
  
  test('does not currently detect PDF from raw bytes', () => {
    expect(getFileMimeFromBase64Data(PDF_HEADER)).toBe('');
  });
  
  test('returns empty string for unknown headers', () => {
    expect(getFileMimeFromBase64Data(new Uint8Array([0x00, 0x01, 0x02, 0x03]))).toBe('');
    expect(getFileMimeFromBase64Data(new Uint8Array(0))).toBe('');
  });
  ...
  test('only inspects the first 8 bytes', () => {
    ...
    expect(getFileMimeFromBase64Data(padded)).toBe('image/png');
  });
});
