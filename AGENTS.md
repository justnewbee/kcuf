...
1. Place under the matching `packages-*/` group; the workspace will pick it up automatically.
2. Choose the right scope: `@kcuf/` (default), `@kcuf-ui/` (UI), `@kcuf-hook/` (hooks). The `kcuf-ui` aggregator package re-exports selected `@kcuf-ui/*` deps — update its `dependencies` if the new package belongs there.
3. Run `pnpm boot` then `pnpm --filter <new-pkg> build` before any other workspace package depends on it.
