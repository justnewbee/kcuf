...
interface IPropsForChoiceGroup<T, V> extends Omit<IPropsChoiceGroup<T, V>, 'defaultValue'> {
  ...
  renderInput(checked: boolean, itemValue: T, onChange: (e: ChangeEvent<HTMLInputElement>, v: T) => void): ReactElement;
}

function renderInputCheckbox<T>(checked: boolean, itemValue: T, onChange: (e: ChangeEvent<HTMLInputElement>, v: T) => void): ReactElement {
  return <input {...{
    type: 'checkbox',
    checked,
...
function renderInputRadio<T>(checked: boolean, itemValue: T, onChange: (e: ChangeEvent<HTMLInputElement>, v: T) => void): ReactElement {
  return <input {...{
    type: 'radio',
    checked,
    onChange: e => onChange(e, itemValue)
...
function ChoiceGroup<T, V = T>({
  ...
  isChecked,
  renderInput
}: IPropsForChoiceGroup<T, V>): ReactElement | null {
...
export function CheckboxGroup<T = string>({
  ...
  defaultValue,
  onChange
}: TPropsCheckboxGroup<T>): ReactElement | null {
  if (!items?.length) {
    return null;
  }
