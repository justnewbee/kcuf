...
作装饰，亦可作按钮（有 `onClick` 时，不需 `button` 包裹，已处理相关 A11Y 的需求）。
...
| --- | --- |
| `Icon` | 组件 |
| `IconRef` | `Ref` 类型，指向底层的 `<i>` 元素，二次封装时传递给 `forwardRef` 的类型 |
| `IconProps` | 组件 `props` 类型，一般用于二次封装 |
| `IconType` | 组件 `type` 的类型<br />`"search" \| "loading" \| "arrowhead" \| "arrow" \| "arrow-to-top" \| "triangle" \| "info" \| "info-fill" \| "help" \| "help-fill" \| "warn" \| "warn-fill" \| "success" \| "success-fill" \| ... 16 more ... \| "external"` |
...
`Icon` 的底层 DOM 元素是 `<i>`，支持其所有 DOM 事件，常用事件为 `onClick`：
