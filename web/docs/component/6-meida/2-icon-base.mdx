...
参考 `@kcuf-ui/icon` 的实现，核心代码 50 行左右：
...
| `size` | `xs \| s \| m \| l \| xl` | 图标大小（字号大小），不设置则继承容器字号，与周边文字和谐共处 |
| `sizeRelative` <TagDefault>false</TagDefault> | `boolean` | 当设置 `size` 时，可以同时设置 `sizeRelative`，此时会用 `em` 相对大小 |
...
| `eyeCatching` <TagDefault>false</TagDefault> | `boolean` | 给图标一个脉搏放大特效，以起到获取用户注意力的效果 |
