...
| `nodeRef` | `RefObject<HTMLElement>` | A React reference to the DOM element that needs to transition. It is only used for do a force reflow.<br /><br />When `mountOnEnter \|\| unmountOnExit`, it will be needed to make transition on enter to take effect. |
