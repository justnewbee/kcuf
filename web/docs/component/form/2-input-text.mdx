...
| `variant` | `'subtle' \| 'underline' \| 'transparent'` | 形态变体 |
...
是的，多数 UI 库都管此组件叫 `Input`，也有叫 `TextField` 的，目前看到只有 [primereact](https://primereact.org/inputtext) 的命名是 `InputText`。

我从来没有借鉴过，很开心看到有人用同样的命名方式。

如此命名的考量，是因为 **前缀命名法** 能够把相关联的物件天然地聚集在一起，从而提升代码的可读性。因为原生的 [HTML input](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input) 实际上有很多类型，`"text"` 仅仅是最常用的一个。
...
| [atlassian](https://atlassian.design/components/textfield/code) | `TextField` | `Forms & input` | `appearance: subtle \| standard \| none` |
...
| [fluent](https://learn.microsoft.com/en-us/fluent-ui/web-components/components/text-field) | `TextField` | - | - |
...
| [next](https://nextui.org/docs/components/input) | `Input` | - | `variant: flat \| bordered \| underlined \| faded` |
...
| [theme](https://theme-ui.com/components/input) | `Input` | - | - |
| [tremor](https://tremor.so/docs/inputs/input) | `Input` | `Inputs` | - |
