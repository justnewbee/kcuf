...
/**
 * 为了使用上的便利性，你可以仅传入内容（string 或 ReactElement），这个方法是判断并返回 `{ content }` 对象
 */
function fromContentOrProps<R = void, D extends object = Record<string, unknown>>(contentOrProps?: string | ReactElement | DialogProps<R, D>): DialogProps<R, D> {
  if (!contentOrProps) {
    return {};
  }
...
export default function buildPropsForPromise<R = void, D extends object = Record<string, unknown>>(
    contentOrProps?: string | ReactElement | DialogProps<R, D>,
