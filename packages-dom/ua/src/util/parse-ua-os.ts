...
export default function parseUaOs(ua: string): [EUaOs, string] {
  ...
  if (/Windows/i.test(ua)) {
    const arr = /Windows NT ([^ /;()]+)/i.exec(ua);
    
    if (arr) {
      const ntVersion = `NT ${arr[1] ?? ''}`;
    ...
    execMatchArr = /Windows Phone[/ ]([^ /;()]+)/i.exec(ua);
    
    if (execMatchArr) {
      return [EUaOs.WINDOWS_PHONE, execMatchArr[1] ?? ''];
    }
  ...
  execMatchArr = /Mac OS X ([^ /;()]+)/i.exec(ua);

  if (execMatchArr) {
    return [EUaOs.MAC_OS, (execMatchArr[1] ?? '').replace(/_/g, '.')];
  }
  ...
  if (/iPhone|iPad/i.test(ua)) {
    execMatchArr = /CPU (?:iPhone )?OS ([^ /;()]+)/i.exec(ua); // iPad 是 CPU OS，iPhone 是 CPU iPhone OS

    if (execMatchArr) {
      return [EUaOs.IOS, (execMatchArr[1] ?? '').replace(/_/g, '.')];
    }
  }
  ...
  if (/Linux/i.test(ua)) {
    ...
    execMatchArr = /Android ([^ /;()]+)+/i.exec(ua);
    
    if (execMatchArr) {
      return [EUaOs.ANDROID, execMatchArr[1] ?? ''];
    }
    ...
    execMatchArr = /Ubuntu\/([^ /;()]+)/i.exec(ua);
    
    if (execMatchArr) {
      return [EUaOs.UBUNTU, execMatchArr[1] ?? ''];
    }
    ...
    execMatchArr = /Fedora\/([^ /;()]+)/i.exec(ua);
    
    if (execMatchArr) {
      return [EUaOs.FEDORA, execMatchArr[1] ?? ''];
    }
    ...
    execMatchArr = /Red Hat\/([^ /;()]+)/i.exec(ua);
    
    if (execMatchArr) {
      return [EUaOs.RED_HAT, execMatchArr[1] ?? ''];
    }
    ...
    execMatchArr = /SUSE\/([^ /;()]+)/i.exec(ua);
    
    if (execMatchArr) {
      return [EUaOs.SUSE, execMatchArr[1] ?? ''];
    }
