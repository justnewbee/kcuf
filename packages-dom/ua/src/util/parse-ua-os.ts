 ...
 * 注意顺序不要轻易改
 */
export default function parseUaOs(ua: string): [EUaOs, string] {
  /**
   * Windows
  ...
  if (/Windows/i.test(ua)) {
    const matchedNt = /Windows NT ([^ /;()]+)/i.exec(ua);

    if (matchedNt) {
      const ntVersion = `NT ${matchedNt[1] ?? ''}`;

      return [EUaOs.WINDOWS, WINDOWS_NT_TO_VERSION[ntVersion] || ntVersion];
    }
    
    // Mozilla/5.0 (compatible; MSIE 10.0; Windows Phone 8.0; Trident/6.0; IEMobile/10.0; ARM; Touch; NOKIA; Lumia 1520)
    const matchedPhone = /Windows Phone[/ ]([^ /;()]+)/i.exec(ua);
    
    if (matchedPhone) {
      return [EUaOs.WINDOWS_PHONE, matchedPhone[1] ?? ''];
    }

    // Opera/9.80 (Windows Mobile; Opera Mini/5.1.21594/37.6270; U; en) Presto/2.12.423 Version/12.16
    if (/Windows Mobile/i.test(ua)) {
      return [EUaOs.WINDOWS_MOBILE, ''];
    }

    return [EUaOs.WINDOWS, ''];
  }

  // MacOS
  // e.g. Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:78.0) Gecko/20100101 Firefox/78.0
  // Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_5) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.1.1 Safari/605.1.15
  const matchedMac = /Mac OS X ([^ /;()]+)/i.exec(ua);

  if (matchedMac) {
    return [EUaOs.MAC_OS, (matchedMac[1] ?? '').replace(/_/g, '.')];
  }
  ...
  if (/iPhone|iPad/i.test(ua)) {
    const matchedIos = /CPU (?:iPhone )?OS ([^ /;()]+)/i.exec(ua); // iPad 是 CPU OS，iPhone 是 CPU iPhone OS

    if (matchedIos) {
      return [EUaOs.IOS, (matchedIos[1] ?? '').replace(/_/g, '.')];
    }
  }
  ...
  if (/Linux/i.test(ua)) {
    ...
    const matchedAndroid = /Android ([^ /;()]+)+/i.exec(ua);

    if (matchedAndroid) {
      return [EUaOs.ANDROID, matchedAndroid[1] ?? ''];
    }
    ...
    const matchedUbuntu = /Ubuntu\/([^ /;()]+)/i.exec(ua);

    if (matchedUbuntu) {
      return [EUaOs.UBUNTU, matchedUbuntu[1] ?? ''];
    }
    ...
    const matchedFedora = /Fedora\/([^ /;()]+)/i.exec(ua);

    if (matchedFedora) {
      return [EUaOs.FEDORA, matchedFedora[1] ?? ''];
    }
    ...
    const matchedRedHat = /Red Hat\/([^ /;()]+)/i.exec(ua);

    if (matchedRedHat) {
      return [EUaOs.RED_HAT, matchedRedHat[1] ?? ''];
    }

    if (/Red Hat/i.test(ua)) {
      return [EUaOs.RED_HAT, ''];
    }

    // SUSE
    // Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.152 Safari/537.36 SUSE/33.0.1750.152
    const matchedSuse = /SUSE\/([^ /;()]+)/i.exec(ua);

    if (matchedSuse) {
      return [EUaOs.SUSE, matchedSuse[1] ?? ''];
    }
