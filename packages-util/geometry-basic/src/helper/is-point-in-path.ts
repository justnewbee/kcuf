 ...
 *
 * 类似 canvas 的 `isPointInPath` API https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/isPointInPath
 *
 ...
 */
export default function isPointInPath(point: TPoint, path: TPath): boolean {
  return path.length <= 2 ? false : getSegmentList(path).reduce((result, v) => {
    const [[xi, yi], [xj, yj]] = v;
