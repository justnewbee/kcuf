...
describe(`${pkgInfo.name}@${pkgInfo.version}`, () => {
  describe('pathPerimeter(path: Path): number', () => {
    ...
    test('Path with 2+ points has segment list of size n', () => {
      ...
      expect(pathPerimeter(PATH_INFO_CONVEX_POLYGON.path)).toEqual(PATH_INFO_CONVEX_POLYGON.perimeter);
      expect(pathPerimeter(PATH_INFO_CONCAVE_POLYGON.path)).toEqual(PATH_INFO_CONCAVE_POLYGON.perimeter);
    });
  });
});