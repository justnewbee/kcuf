...
export default function utob(u: string): string {
  return u.replace(REG_UTOB, v => {
    if (v.length < 2) {
      ...
      return cc < 0x80 ? v : cc < 0x800 ? fromCharCode(0xc0 | (cc >>> 6)) + fromCharCode(0x80 | (cc & 0x3f)) : fromCharCode(0xe0 | ((cc >>> 12) & 0x0f)) + fromCharCode(0x80 | ((cc >>> 6) & 0x3f)) + fromCharCode(0x80 | (cc & 0x3f)); // eslint-disable-line max-len
    }
